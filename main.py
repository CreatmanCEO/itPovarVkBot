...
def handle_event(event):
    ...
    print(f"Получено сообщение: {text} от {user_name} ({user_id})")  # Для отладки

    # Приветственное сообщение при пустом тексте
    if text == "":
        message = (f"Здравствуйте, {user_name}! Я автоматический помощник сообщества — IT-Помощь в Поварово. "
    ...
    if text in ["начать", "привет", "здравствуйте", "добрый день", "добрый вечер"]:
        ...
        send_message(user_id, (f"Извините, {user_name}, я не понял ваш запрос. Попробуйте выбрать одну из опций: "
                               "'Услуги Населению' или 'Услуги для Бизнеса'."))
...
def handle_event(event):
    ...
    if text == "":
        message = (f"Здравствуйте, {user_name}! Я автоматический помощник сообщества — IT-Помощь в Поварово. "
                   "Мы занимаемся настройкой компьютеров, разработкой программ и приложений, установкой программ, обучением работе с ИИ "
                   "и другими IT-услугами для частных лиц и бизнеса. Чем я могу вам помочь?")
        keyboard = create_keyboard(["Услуги Населению", "Услуги для Бизнеса"])
        send_message(user_id, message, keyboard)
        return
    ...
    if text in ["начать", "привет", "здравствуйте", "добрый день", "добрый вечер"]:
        ...
        message = ("Мы предлагаем услуги:\n"
                   "- Настройка и мелкий ремонт компьютеров и мобильных устройств\n"
                   "- Установка программ, антивирусов, востановление данных\n"
                   "- Разработка сайтов, ботов, приложений и автоматизаций\n"
                   "- Обучение програмированию, работе с программами и многое другое!\n\n"
                   "Что случилось и как мы можем вам помочь?")
        send_message(user_id, message)
    elif text == "услуги для бизнеса":
        message = ("Пожалуйста, можете ли вы вкратце описать деятельность вашей компании?")
